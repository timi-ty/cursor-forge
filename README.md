...
|-------|-------------|----------|--------------|
| **[development-harness](skills/development-harness/)** | Project-local control plane that compiles ROADMAP.md into phased, validator-backed autonomous execution. 7 commands: create, invoke, update, state, sync, clear, inject-issues. | `create dev harness`, `invoke harness`, `continue from harness`, `harness state`, `sync harness`, `clear harness`, `inject issues` | `git`, `python3` |
